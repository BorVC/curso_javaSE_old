...
public class PedidosView {
	...
	static void agregarPedido() throws ParseException {
		...
		LocalDate fecha=LocalDate.parse(scn.nextLine(), format);//Pasar de cadena a fecha
	...
	static void pedidoUltimo() {
		...
		System.out.println("fecha: " + pedido.getFecha().format(null));
	}
	...
	static void pedidoFechas() throws ParseException {
		...
		LocalDate fecha = LocalDate.parse(scn.nextLine(),sdf);//Pasar de cadena a fecha
		System.out.println("Fecha segunda dd/MM/yyyy:");
		LocalDate fecha2 = LocalDate.parse(scn.nextLine(),sdf);//Pasar de cadena a fecha
		ArrayList<Pedidos> pedidos = service.pedidosFechas(fecha, fecha2);
		for(Pedidos pedido: pedidos) {
			System.out.println("Producto : " + pedido.getProducto());
			System.out.println("Unidades: " + pedido.getUnidades());
			System.out.println("fecha: " + pedido.getFecha().format(sdf));
			System.out.println("---------------------");
			}
	}
